...
- CLI: `vibe chat` has **opt-in LLM enhancement** (owner-approved): if `OPENAI_API_KEY`/`ANTHROPIC_API_KEY` is present it generates the plan + prompt via the provider (native `fetch`, zero SDK deps — `src/utils/llm.ts`; `VIBE_PROVIDER`/`VIBE_MODEL` override). Everything else is local heuristics. **Hard rules:** never send source file contents (only the task, answers and AGENTS.md stack/standards sections); always fall back to heuristics on any API failure; do not add more API calls without prior discussion. Keep non-interactive (no-TTY) flows working: CI exercises them (`init -y`, `optimize`, `chat`, `review -f`).
