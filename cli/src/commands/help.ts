...
const TOPICS: Record<string, CommandHelp> = {
  ...
  context: {
    ...
    description:
      'Re-scans the project and rewrites AGENTS.md and .iderules so they never go stale. Detects monorepos — npm/yarn/pnpm/lerna workspaces, or inferred from the directory structure when no tooling is declared. When an API key is present, AGENTS.md is generated by the LLM from the project scan; use --no-llm to force local heuristics. With --describe, an LLM rewrites AGENTS.md from your description plus the scan; only the description and stack summary are sent.',
    ...
    tips: ['Use --dry-run liberally; it never touches your files.', 'Use --describe to realign AGENTS.md with a new product direction.', 'Use --no-llm in CI or when you do not want any LLM calls.'],
  },
