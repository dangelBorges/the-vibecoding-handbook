...
import { detectLlmConfig, generateText, type LlmConfig } from '../utils/llm.js';
...
// Extracts the Tech Stack and Coding Standards sections from AGENTS.md (if any).
// This is the only project context ever sent to an LLM — never file contents.
function getAgentsMdContext(): string {
...
function generateOptimizedPrompt(session: ChatSession, scan: ProjectScan): string {
  const context = getAgentsMdContext();
...
const PLAN_DELIMITER = '=== PLAN ===';
const PROMPT_DELIMITER = '=== PROMPT ===';

function buildLlmRequest(session: ChatSession, scan: ProjectScan): { system: string; user: string } {
  const context = getAgentsMdContext();
  const answers = session.messages
    .filter((m) => m.role === 'user')
    .map((m) => `- ${m.content}`)
  ...
  const system =
    'You are a senior software architect helping plan a development task. ' +
    'Reply in concise, actionable Markdown. Follow the requested output format exactly.';

  const user = [
    `Task topic: ${session.topic}`,
    `Project stack: ${scan.framework} + ${scan.language}`,
    context ? `Project context:\n${context}` : '',
    `User input:\n${answers}`,
    'Produce exactly two artifacts, in this order, with these exact delimiters and no text outside them:',
    PLAN_DELIMITER,
    'A step-by-step implementation plan: understanding of the task, numbered implementation steps, and a short risk checklist.',
    PROMPT_DELIMITER,
    'An optimized prompt the user can paste into an AI coding agent to implement the task: role, task description, constraints, and expected output.',
  ]
    .filter(Boolean)
    .join('\n\n');
...
// Returns null when the LLM is unreachable or the response misses the delimiters.
async function generateLlmOutputs(
  session: ChatSession,
  scan: ProjectScan,
  config: LlmConfig
): Promise<{ plan: string; prompt: string } | null> {
  const { system, user } = buildLlmRequest(session, scan);
  const text = await generateText(config, system, user);
  if (!text) return null;
  const plan = text.split(PLAN_DELIMITER)[1]?.split(PROMPT_DELIMITER)[0]?.trim();
  const prompt = text.split(PROMPT_DELIMITER)[1]?.trim();
...
export async function chatCommand(arg?: string | string[]): Promise<void> {
  ...
  // Generate plan and optimized prompt (LLM-enhanced when an API key is present)
  section('Generating outputs');

  const llm = detectLlmConfig();
  if (llm) {
    info(`AI mode: ${llm.provider} (${llm.model})`);
  } else {
    info('Local mode: heuristic generation (no API key detected)');
  }

  const llmOutputs = llm ? await generateLlmOutputs(session, scan, llm) : null;
  if (llm && !llmOutputs) {
    warn('LLM unavailable — falling back to local heuristics');
  }
  const plan = llmOutputs?.plan ?? generatePlan(session);
  const optimizedPrompt = llmOutputs?.prompt ?? generateOptimizedPrompt(session, scan);
