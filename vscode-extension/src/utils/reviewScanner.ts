...
function checkConsoleLog(file: string, lines: string[], ideRules: string, agentsMd: string): ReviewIssue[] {
  const noConsoleRule =
    ideRules.toLowerCase().includes('console.log') ||
    agentsMd.toLowerCase().includes('console.log') ||
    ideRules.length > 0;
  ...
  lines.forEach((line, idx) => {
    if (line.includes(': any') || line.includes('as any')) {
      issues.push({
        ...
        line: idx + 1,
        severity: 'warning',
        message: '`any` type detected — use specific type',
        rule: 'No any types',
      });
    }
  });
    ...
    for (const pattern of secretPatterns) {
      if (line.includes(pattern) && !line.includes('process.env') && !line.includes('import')) {
        issues.push({
          ...
          line: idx + 1,
          severity: 'error',
          message: `Possible hardcoded secret: "${pattern}" — use environment variables`,
  ...
  lines.forEach((line, idx) => {
    if (line.includes('.then(') && line.includes('.catch(')) {
      issues.push({
        ...
        line: idx + 1,
        severity: 'info',
        message: 'Consider using async/await instead of .then().catch()',
    ...
    if (funcStart >= 0 && line === '}') {
      ...
      if (length > 50) {
        issues.push({
          ...
          line: funcStart + 1,
          severity: 'info',
          message: `Function is ${length} lines — consider extracting (${length - 50} over limit)`,
          rule: 'Max 50 lines per function',
        });
      }
      funcStart = -1;
    }
